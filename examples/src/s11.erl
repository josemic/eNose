...
s()->
    ...
    %%dbg:p(new, m),
    %%dbg:p(new, p),
    {GotoDictNew, FailDict, OutputDictNew} = ahocorasick:trie([<<"meldung">>, <<"thema">>]),
    ...
    {ok, Result1} = rule:start([{epcap_port,[{interface, "eth0"}, {filter, "tcp"}]}, {defrag, []}, {content, [{matchfun, MatchFun1}, {message, "Found: *meldung* oder *thema*"}]}]),
