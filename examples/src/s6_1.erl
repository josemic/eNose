...
s()->
    ...
    {ok, Result1} = rule:start([{epcap_port,[{interface, "eth1"}, {filter, "tcp"}]}, {stream, []}, {content, [{matchfun, MatchFun1}, {message, "Found: GET*Meldung*"}]}]),
    ...
    MatchFun2 = fun(Payload) -> 
			...
			E = parser_combinator_bitstring:pBetweenN(B, C,14),
			parser_combinator_bitstring:parse(E,Payload) end,
    {ok, Result2} = rule:start([{epcap_port,[{interface, "eth1"}, {filter, "tcp"}]}, {stream, []}, {content, [{matchfun, MatchFun2}, {message, "Found: GET*Thema*"}]}]),
