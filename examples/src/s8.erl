...
s()->
    ...
    {ok, Result1} = rule:start([{epcap_port,[{interface, "eth0"}, {filter, "tcp"}]}, {stream, []}, {content, [{matchfun, MatchFun1}, {message, "Found: GET*Meldung*"}]}]),
    io:format("Start result 1: ~p~n",[Result1]),
    AlwaysMatchFun = fun(Payload) -> no_fail end,
    {ok, Result2} = rule:start([{epcap_port,[{interface, "eth0"}, {filter, "tcp"}]}, {stream, []}, {content, [{matchfun, AlwaysMatchFun}, {message, "Received package:"}]}]),
